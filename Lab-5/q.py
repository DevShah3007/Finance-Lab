...
def execute_model(stocks_name, type, mu_market_index, risk_market_index):
  ...
  for i in range(len(stocks_name)):
    filename = type + '/' + stocks_name[i] + '.csv'
    df = pd.read_csv(filename)
    df.set_index('Date', inplace=True)
...
def main():
  ...
  mu_market_BSE, risk_market_BSE = find_market_portfolio('./SENSEX/BSE.csv')



  # =====================  sub-part (ii)  =====================
  print("\n\n********************  Market portfolio for NSE using Index  ********************")
  mu_market_NSE, risk_market_NSE = find_market_portfolio('./NIFTY/NSE.csv')
  ...
  stocks_name = ['HDFCBANK.BO','ICICIBANK.BO','INFY.BO','LT.BO','ONGC.BO','RELIANCE.BO','TATAMOTORS.BO','TATASTEEL.BO','TCS.BO','WIPRO.BO']
  execute_model(stocks_name, 'SENSEX', mu_market_BSE, risk_market_BSE)
  ...
  stocks_name = ['HDFCBANK.NS','ICICIBANK.NS','INFY.NS','ITC.NS','LT.NS','RELIANCE.NS','SBIN.NS','TCS.NS','TITAN.NS','WIPRO.NS']
  execute_model(stocks_name, 'NIFTY', mu_market_NSE, risk_market_NSE)



  # =====================  sub-part (v)  =====================
  print("\n\n********************  10 stocks not from BSE Index  ********************")

  stocks_name = ['HBLPOWER.BO','ISGEC.BO','JPASSOCIAT.BO','MIRZAINT.BO','NBCC.BO','PNB.BO','PNBGILTS.BO','SUZLON.BO','UNITINT.BO','YESBANK.BO']
  execute_model(stocks_name, 'NON-SENSEX', -1, -1)

  print("\n\n********************  10 stocks not from NSE Index  ********************")

  stocks_name = ['AARTIDRUGS.NS','APARINDS.NS','BEL.NS','ECLERX.NS','HDFCLIFE.NS','LUXIND.NS','NHPC.NS','NOCIL.NS','RADICO.NS','VMART.NS']
  execute_model(stocks_name, 'NON-NIFTY', -1, -1)
